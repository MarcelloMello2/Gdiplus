...
{ Realçador de sintaxe/html do "pobre homem" }
...
procedure SourceCodeToRichText(const Title: String; const SourceCode: TStrings;
  const RichEdit: TRichEdit; FileName: string);
...
const
  GdiplusTypesNames: array[0..10] of String = (
    'TGdipGraphics', 'TGdipPen', 'TGdipColor',
    'TGdipBrush', 'TGdipSolidBrush', 'TGdipFont',
    'TGdipFontFamily', 'TGdipGraphicsUnit', 'TGdipFontStyle',
    'TGdipImage', 'TGdipTextureBrush');

var
  ReservedWords: TStringList = nil;
  GdiplusTypes: TStringList = nil;
...
begin
  ...
  GdiplusTypes := TStringList.Create;
  GdiplusTypes.Sorted := True;
  GdiplusTypes.Duplicates := dupError;
  for S in GdiplusTypesNames do
    GdiplusTypes.Add(S);
end;

procedure Finalize;
begin
  FreeAndNil(GdiplusTypes);
    ...
    case P^ of
      ...
      '_', 'A'..'Z', 'a'..'z':
        begin
          ...
          else if (GdiplusTypes.IndexOf(Q) >= 0) then
            Add(seGdiplusType, Q)
          else
            Add(seSymbol, Q);
          P^ := C;
...
procedure SourceCodeToRichText(const Title: String; const SourceCode: TStrings; const RichEdit: TRichEdit; FileName: string);
var
  Attr: TTextAttributes;
  S, Trimmed: String;
  InRegion: Boolean;
